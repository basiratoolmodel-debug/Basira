...
def api_get(path: str):
    try:
        return requests.get(f"{LOCAL_API}{path}", timeout=15)
    except Exception:
        return None
...
def api_post(path: str, payload=None):
    try:
        return requests.post(f"{LOCAL_API}{path}", json=payload or {}, timeout=25)
    except Exception:
        return None
...
def pick_folder_from_system():
    response = api_get("/api/system/pick-data-dir")
    if response and response.ok:
...
def render_new_user_setup():
    ...
    if not st.session_state.data_dir:
        st.session_state.data_dir = str(Path.home() / "Documents" / "BasiraData")
    ...
    with col2:
        if st.button("استعراض", use_container_width=True):
            selected = pick_folder_from_system()
            if selected:
    ...
    if st.button("حفظ المسار والبدء بالتجهيز", use_container_width=True):
        with st.spinner("جارٍ إنشاء المجلدات الأساسية..."):
            response = configure_data_dir(st.session_state.data_dir)
...
def render_recovery(state):
    ...
    if reason in ["config_corrupted", "config_invalid"]:
        ...
        if "recovery_data_dir" not in st.session_state or not st.session_state.recovery_data_dir:
            st.session_state.recovery_data_dir = str(Path.home() / "Documents" / "BasiraData")

        col1, col2 = st.columns([5, 1])

        with col1:
            st.text_input(
                "اختيار Data Directory جديد",
                value=st.session_state.recovery_data_dir,
                key="recovery_dir_display",
                disabled=True
            )

        with col2:
            if st.button("استعراض", key="browse_recovery_dir", use_container_width=True):
                selected = pick_folder_from_system()
                if selected:
                    st.session_state.recovery_data_dir = selected
                    st.rerun()

        if st.button("تحديث مسار البيانات", use_container_width=True):
            response = reselect_data_dir(st.session_state.recovery_data_dir)
            if response and response.ok:
                st.success("تم تحديث مسار البيانات.")
                time.sleep(1)
                refresh_startup_status()
