...
Basira Launcher
===============
This file runs automatically:
  - On first install: triggered by Install_Basira.bat
  - On every Windows login: registered in startup registry
  - From desktop shortcut: Basira.lnk

The customer NEVER touches a terminal or types any command.

What it does:
  1. Registers itself in Windows startup (once, silently)
  2. Starts basira_local_bootstrap.py  →  http://127.0.0.1:5001
  3. Starts Basira_app_structure.py    →  http://127.0.0.1:5000
  4. Opens the browser:
       - First time: to the cloud setup page for folder selection & download
       - Every time after: directly to 127.0.0.1:5000 (analysis UI)
"""
...
def register_startup():
    """Add launcher to Windows startup so it runs on every login. Silent."""
    if os.name != "nt":
        return
    try:
        import winreg
        # Prefer pythonw.exe (no console window shown to user)
...
def silent_popen(cmd: list) -> subprocess.Popen:
    """Launch with no visible console window on Windows."""
    kwargs: dict = {"cwd": str(APP_ROOT)}
    if os.name == "nt":
        si = subprocess.STARTUPINFO()
        si.dwFlags |= subprocess.STARTF_USESHOWWINDOW
...
# ─── Bootstrap init (idempotent) ─────────────────────────────────────────────
def init_bootstrap():
    """Creates config.json in AppData if missing. Safe to call every launch."""
    try:
        req = urllib.request.Request(
            f"http://127.0.0.1:{BOOTSTRAP_PORT}/api/setup/init",
            data=b"{}",
...
def get_startup_state() -> str:
    """Ask the bootstrap what state the installation is in."""
    try:
...
def decide_url() -> str:
    """
    - If setup is complete and healthy → open local app directly (127.0.0.1:5000)
    - If first time or incomplete     → open cloud setup page for folder selection
    """
    ...
    if state in ("healthy", "healthy_with_optional_update"):
        ...
        # First time or needs setup: open the cloud setup page
        # The cloud page talks to :5001 and guides the user through folder selection + download
        return CLOUD_SETUP_URL
...
def main():
    ...
    log("=" * 52)
    log("Basira launching...")
    log("=" * 52)

    # 1. Register in Windows startup (once, silent)
    if not is_startup_registered():
        register_startup()

    # 2. Start bootstrap API (:5001)
    start_bootstrap()
    if not wait_for_port(BOOTSTRAP_PORT, timeout=25):
        raise RuntimeError(f"Bootstrap failed to start on :{BOOTSTRAP_PORT}")
    log(f"Bootstrap ready → http://127.0.0.1:{BOOTSTRAP_PORT}")

    # 3. Init config (idempotent — creates AppData/Basira/config.json if missing)
    init_bootstrap()

    # 4. Start main Flask app (:5000)
    start_main_app()
    if not wait_for_port(MAIN_APP_PORT, timeout=30):
    ...
    # 5. Open correct URL
    #    Background mode (startup): do NOT open browser — user didn't click anything
    if not background:
        url = decide_url()
        webbrowser.open(url)
        log(f"Browser opened  → {url}")
