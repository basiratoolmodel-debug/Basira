...
from basira_paths import default_user_data_dir
...
def build_default_config(data_dir: str = "", user_id: str = ""):
    if not data_dir:
        data_dir = str(default_user_data_dir())
...
def check_session(config: dict):
    ...
    if last_activity_at:
        try:
            ...
            minutes_idle = (datetime.now(timezone.utc) - last_activity_dt).total_seconds() / 60
            if minutes_idle >= SESSION_TIMEOUT_MINUTES:
                return {
                    "valid": False,
