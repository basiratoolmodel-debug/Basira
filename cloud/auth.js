...
async function completeLoginSuccess(userName, sub) {
  ...
  if (session?.user?.id) {
    localStorage.setItem("basira_user_id", session.user.id);
  }

  if (session?.access_token) {
    localStorage.setItem("basira_access_token", session.access_token);
  }

  if (session?.refresh_token) {
    localStorage.setItem("basira_refresh_token", session.refresh_token);
  }

  if (session?.expires_at) {
    const expiresIso = new Date(session.expires_at * 1000).toISOString();
    localStorage.setItem("basira_session_expires_at", expiresIso);
  }
  ...
  const popupText = document.getElementById("loginSuccessPopupText");
  if (popup && popupText) {
    popupText.textContent = `أهلًا ${userName}، تم تسجيل الدخول بنجاح وسيتم تجهيز بيئتك المحلية الآن.`;
    popup.classList.add("show");
  }

  showNote("loginMessage", "ok", "تم تسجيل الدخول بنجاح. سيتم تحويلك الآن إلى تهيئة البيئة المحلية.");
  hideSubscriptionNotice();

  setTimeout(() => {
    window.location.href = "local-setup.html";
  }, 1500);
}
