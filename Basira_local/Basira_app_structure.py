...
def health():
    ...
    """
    def _get_scraper_port():
        try:
            port_file = Path(__file__).resolve().parent / "scraper.port"
    ...
    scraper_port = _get_scraper_port()
    return jsonify({
        "status":      "ok",
        "server_time": now_iso(),
        ...
        "services": {
            ...
            "preprocessor": _is_port_open(5050),
            "analysis":     _is_port_open(5055),
            "scraper":      scraper_port is not None,
        },
    })
...
def launch_preprocessor():
    ...
    # حاول launcher_preprocessor.py أولاً، ثم launch_preprocessor.py، وإلا basira_app.py مباشرة
    launcher = (
        _find_launcher("launcher_preprocessor.py")
        or _find_launcher("launch_preprocessor.py")
        or _find_launcher("basira_app.py")
    )
    if launcher is None:
        return jsonify({
            "status": "error",
            "message": "launcher_preprocessor.py / launch_preprocessor.py / basira_app.py not found. تأكد من وجود الملفات في templates/",
            "searched_in": [
                str(TEMPLATES_DIR / "launcher_preprocessor.py"),
                str(TEMPLATES_DIR / "launch_preprocessor.py"),
                str(TEMPLATES_DIR / "basira_app.py"),
                str(Path(__file__).resolve().parent / "launcher_preprocessor.py"),
            ],
        }), 404
