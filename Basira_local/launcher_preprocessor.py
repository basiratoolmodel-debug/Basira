...
FIX v1.1:
  - cwd الآن = مجلد templates/ (كان BASE_DIR وكانت
    مجلدات uploads/ و outputs/ تُنشأ في المكان الخاطئ)
"""
...
# ── الإعدادات ─────────────────────────────────────────────────
PREPROCESSOR_SCRIPT = os.path.join(BASE_DIR, "templates", "basira_app.py")
PREPROCESSOR_DIR    = os.path.join(BASE_DIR, "templates")   # FIX: cwd الصحيح
...
def start_preprocessor():
    ...
    log.info(f"cwd = {PREPROCESSOR_DIR}")

    # FIX: cwd = templates/ حتى تُنشأ uploads/ و outputs/ في المكان الصحيح
    log_path = os.path.join(os.path.dirname(LOG_FILE), "preprocessor_stderr.log")
    with open(log_path, "a", encoding="utf-8") as stderr_file:
        import datetime as _dt
        stderr_file.write(f"\n{'='*60}\n")
        stderr_file.write(f"[{_dt.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] Preprocessor launch attempt\n")
