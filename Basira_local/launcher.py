...
def silent_popen(cmd: list, cwd: str = None) -> subprocess.Popen:
    ...
    kwargs: dict = {"cwd": cwd or str(INSTALL_DIR)}
    if os.name == "nt":
...
def start_preprocessor():
    ...
    silent_popen([sys.executable, str(target), "--silent"], cwd=str(target.parent))
...
def start_scraper():
    ...
    silent_popen([sys.executable, str(SCRAPER_LAUNCHER), "--silent"], cwd=str(SCRAPER_LAUNCHER.parent))
...
def start_analysis():
    ...
    silent_popen([sys.executable, str(ANALYSIS_LAUNCHER), "--silent"], cwd=str(ANALYSIS_LAUNCHER.parent))
