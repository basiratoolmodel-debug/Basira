...
PREPROCESSOR_FILE   = TEMPLATES_DIR / "basira_app.py"
PREPROCESSOR_LAUNCHER = TEMPLATES_DIR / "launch_preprocessor.py"
...
def start_main_app():
    current_hash = get_file_hash(MAIN_APP_FILE)

    if is_port_open(MAIN_APP_PORT):
        # Server is running — only restart if the file changed
        if current_hash and current_hash == get_running_hash():
            log(f"Main app already running on :{MAIN_APP_PORT} (no changes)")
            return
        log(f"Main app file changed — restarting on :{MAIN_APP_PORT}...")
        kill_port(MAIN_APP_PORT)
        time.sleep(1.5)
    ...
    save_running_hash(current_hash)
...
def start_preprocessor():
    if is_port_open(PREPROCESSOR_PORT):
        ...
        return
    # Run launch_preprocessor.py which handles deps + starts basira_app.py
    ...
    if not target.exists():
        ...
        return
    log(f"Starting preprocessor via: {target.name}")
    exe = Path(sys.executable)
    python_exe = exe.parent / "python.exe"
    if not python_exe.exists():
        python_exe = exe
    cwd = str(TEMPLATES_DIR) if target.parent == TEMPLATES_DIR else str(INSTALL_DIR)
    kwargs = {"cwd": cwd}
    if os.name == "nt":
        si = subprocess.STARTUPINFO()
        si.dwFlags     = subprocess.STARTF_USESHOWWINDOW
...
def main():
    ...
    start_preprocessor()
    wait_for_port(PREPROCESSOR_PORT, timeout=15)
