...
@app.route("/launch-preprocessor", methods=["POST"])
def launch_preprocessor():
    import subprocess, sys, os
    from pathlib import Path
    server = Path(__file__).resolve().parent / "basira_app.py"
    if not server.exists():
        return jsonify({"status":"error","message":"basira_app.py not found"}), 404
    kwargs = {"cwd": str(server.parent)}
    if os.name == "nt":
...
# ─────────────────────────────────────────────────────────────
# /open-preprocessor — يُستدعى من basira_app.html تلقائياً
# عند الضغط على زر Preprocessor.
# يتحقق إذا السيرفر شغّال، وإلا يشغّله في الخلفية وينتظر
# حتى يصبح جاهزاً قبل الرد — لا يحتاج المستخدم فعل أي شيء.
# ─────────────────────────────────────────────────────────────
...
def open_preprocessor():
    ...
    # السيرفر شغّال مسبقاً — لا حاجة لفعل شيء
    if _port_open(PREPROCESS_PORT):
        return jsonify({"status": "already_running"})

    # شغّل basira_app.py كعملية مستقلة في الخلفية
    ...
    if os.name == "nt":
        si = subprocess.STARTUPINFO()
        si.dwFlags     = subprocess.STARTF_USESHOWWINDOW
        si.wShowWindow = 6   # SW_MINIMIZE — نافذة مصغّرة في الشريط
        kwargs["startupinfo"] = si

    subprocess.Popen([sys.executable, str(server_file)], **kwargs)

    # انتظر حتى 10 ثوانٍ حتى يصبح السيرفر جاهزاً
    for _ in range(20):
        time.sleep(0.5)
        if _port_open(PREPROCESS_PORT):
