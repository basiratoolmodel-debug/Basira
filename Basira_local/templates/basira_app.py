...
def _auto_detect(df: pd.DataFrame, col_types: dict) -> tuple:
    ...
    _EXCLUDE = (
        '_missing', '_flag', '_is_weekend', '_year', '_month', '_day',
        '_hour', '_dow', '_encoded', '_label_enc', 'total_', '_total',
        'count_', '_count', 'num_', '_num', 'assignment', 'timestamp',
    )
    ...
    for col in df.columns:
        ...
        if any(pat in cn for pat in _EXCLUDE):
            continue
        score = 0
        ...
        n_u = df[col].nunique()
        if n_u == 2:
            score += 6   # strongly prefer binary targets
        elif 3 <= n_u <= 10:
            score += 2
        elif n_u > 100:
            score -= 2
        # penalise high-cardinality categorical unless it matched a keyword
        if ctype == "categorical" and n_u > 10 and score < 15:
            score -= 3
        if score > best_score:
...
def index():
    ...
    return redirect("/basira_preprocessor.html")
...
def serve_preprocessor():
    ...
    folder = str(templates) if (templates / "basira_preprocessor.html").exists() else str(base)
    return send_from_directory(folder, "basira_preprocessor.html")



# ─────────────────────────────────────────────────────────────
# /launch-preprocessor  &  /open-preprocessor
# كلاهما يؤديان نفس المهمة: يتحققان من port 5050 ويشغّلان
# basira_app.py إذا لم يكن شغّالاً، ثم ينتظران حتى يجاوب.
# /launch-preprocessor : مبقى للتوافق مع الـ frontend القديم
# /open-preprocessor   : نسخة موسّعة مع OPTIONS preflight
# ─────────────────────────────────────────────────────────────
def _start_preprocessor_server():
    """Core logic shared by both preprocessor launch routes."""
    import socket as _socket, time as _time, subprocess as _subprocess
    ...
    if _port_open(PREPROCESS_PORT):
        return jsonify({"status": "already_running"})

    server_file = Path(__file__).resolve()
    kwargs = {"cwd": str(server_file.parent)}
    ...
    _subprocess.Popen([sys.executable, str(server_file)], **kwargs)
    ...
    return jsonify({
        "status": "timeout",
        "message": "Preprocessor did not respond within 10 seconds"
    }), 503


@app.route("/launch-preprocessor", methods=["POST", "OPTIONS"])
def launch_preprocessor():
    """Compatibility alias — same logic as /open-preprocessor."""
    from flask import Response as _Resp
    if request.method == "OPTIONS":
        return _Resp(status=200)
    return _start_preprocessor_server()


@app.route("/open-preprocessor", methods=["POST", "OPTIONS"])
def open_preprocessor():
    from flask import Response as _Resp
    if request.method == "OPTIONS":
        return _Resp(status=200)
    return _start_preprocessor_server()
...
def upload_process():
    ...
    uid   = _dt.now().strftime("%H%M%S")   # time-based, no random
    ...
    try:
        ...
        _tb.print_exc()  # log to server console only
        return jsonify({"error": "An error occurred during preprocessing. Please check your file and try again."}), 500

    base = re.sub(r'[^a-zA-Z0-9_\-]', '_', fname.rsplit(".", 1)[0])[:40]

    cleaned_path = OUTPUT_DIR / f"Basira_{_date}_{base}_cleaned.csv"
    result["cleaned_df"].to_csv(str(cleaned_path), index=False, encoding="utf-8-sig")

    model_path = OUTPUT_DIR / f"Basira_{_date}_{base}_model_ready.csv"
    ...
    audit_path = OUTPUT_DIR / f"Basira_{_date}_{base}_audit.html"
    audit_path.write_text(audit_html, encoding="utf-8")

    config_path = OUTPUT_DIR / f"Basira_{_date}_{base}_config.json"
...
# ═══════════════════════════════════════════════════════════════
# Scraper Dynamic-Port Routes
# يُتيح للـ preprocessor HTML التحكم في السكرابر ديناميكياً
# ═══════════════════════════════════════════════════════════════

# ملف مؤقت يكتبه launch_webscraping.py بعد تشغيل السكرابر
_SCRAPER_PORT_FILE = Path(__file__).resolve().parent.parent / "scraper.port"


def _read_scraper_port():
    """يقرأ البورت من scraper.port ويتحقق أن السيرفر نشط."""
    import socket as _sock
    for candidate in [
        Path(__file__).resolve().parent.parent / "scraper.port",  # Basira_app/
        Path(__file__).resolve().parent / "scraper.port",          # templates/
    ]:
        try:
...
@app.route("/scraper-port")
def scraper_port_route():
    """يرجع البورت الحالي للسكرابر."""
    port, _ = _read_scraper_port()
    if port:
        return jsonify({"port": port})
    return jsonify({"port": None, "error": "Scraper not running"}), 404


@app.route("/scraper-stop", methods=["POST"])
def scraper_stop_route():
    """يُغلق السكرابر ويحذف scraper.port."""
    import subprocess as _sp, os as _os
    port, port_file = _read_scraper_port()
    if not port:
        ...
        if _os.name == "nt":
            ...
            for line in result.stdout.strip().splitlines():
                ...
                if parts and parts[-1].isdigit():
                    _sp.run(f"taskkill /PID {parts[-1]} /F",
...
@app.route("/scraper-launch", methods=["POST"])
def scraper_launch_route():
    """يشغّل launch_webscraping.py ويرجع البورت بعد الجاهزية."""
    import subprocess as _sp, os as _os, time as _time
    import socket as _sock

    base = Path(__file__).resolve().parent
    # ابحث في Basira_app/ ثم templates/
    candidates = [
        base.parent / "launch_webscraping.py",
        base / "launch_webscraping.py",
    ]
    launcher = next((p for p in candidates if p.exists()), None)
    if not launcher:
        return jsonify({"status": "error",
                        "message": "launch_webscraping.py not found"}), 404

    # تحقق إذا السكرابر شغّال مسبقاً
    existing_port, _ = _read_scraper_port()
    if existing_port:
        return jsonify({"status": "already_running", "port": existing_port})
    ...
    _sp.Popen([str(python_exe), str(launcher), "--silent"], **kwargs)

    # انتظر حتى 15 ثانية حتى يُكتب scraper.port
    for _ in range(30):
        _time.sleep(0.5)
        port, _ = _read_scraper_port()
        if port:
    ...
    return jsonify({"status": "timeout",
                    "message": "Scraper did not start within 15s"}), 503
...
if __name__ == "__main__":
    ...
    if not silent:
        print("🚀 Basira Smart Preprocessor v3.0 — http://127.0.0.1:5050")
