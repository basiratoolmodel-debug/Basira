...
// async function linkToMainApp() {
//   const p = getSession();
//   if (!p.user_id || !p.access_token) return;

//   // Store in localStorage as backup
//   localStorage.setItem("basira_user_id",             p.user_id);
//   localStorage.setItem("basira_access_token",        p.access_token);
//   localStorage.setItem("basira_refresh_token",       p.refresh_token || "");
//   localStorage.setItem("basira_session_expires_at",  p.expires_at || "");
//   localStorage.setItem("basira_subscription_status", p.subscription_status || "active");
// }

// function openLocalAppWithSession() {
//   /**
//    * THE REAL SESSION FIX
//    * ====================
//    * Instead of cross-origin fetch (which loses cookies), we redirect
//    * the browser directly to:
//    *   http://127.0.0.1:5000/login?token=XXX&user_id=YYY&sub=active
//    *
//    * Flask receives this as a same-origin GET request, sets a real
//    * session cookie, then redirects to / (basira_app.html).
//    * The session cookie persists — basira_app.html loads authenticated.
//    *
//    * No CORS. No no-cors. No cookie issues. Works every time.
//    */
//   const p = getSession();
//   if (!p.user_id || !p.access_token) {
//     // No session — just open local app, it will show the overlay
...
//     user_id: p.user_id,
//     sub:     p.subscription_status || "active",
//     refresh: p.refresh_token || "",
//   });

//   // Navigate to /login — Flask sets session cookie, redirects to /
//   window.open(`${LOCAL_APP}/login?${params.toString()}`, "_blank");
// }
...
//     if(readyFolder) { readyFolder.classList.remove("isHidden"); readyFolder.textContent=dataDir; }
//     showNote("ok","✓ تم إعداد بصيرة بنجاح. جارٍ فتح التطبيق...");
//     setTimeout(()=>{ openLocalAppWithSession(); }, 1000);
...
//       if(sub) sub.textContent = "بصيرة جاهزة. جارٍ فتح التطبيق...";
//       showNote("ok","البيئة المحلية جاهزة. جارٍ فتح بصيرة...");
//       setTimeout(()=>{ openLocalAppWithSession(); }, 600);
//       break;
//     }
 ...
 * local-setup.js — Basira On-Premise Setup
 *
 * HOW SESSION WORKS (the correct way):
 * =====================================
 * 1. User logs in via Supabase on the cloud site
 * 2. Supabase stores tokens in its own internal storage (NOT localStorage)
 * 3. We read tokens via supabaseClient.auth.getSession()
 * 4. When ready to open local app, we navigate to:
 ...
 *
 * No CORS. No localStorage bridge. No no-cors fetch. No cookies issues.
 */
...
let _supaSession    = null;   // cached Supabase session
let startupState    = null;
let inactivityTimer = null;
let dlStartTime     = null;
...
function setProgress(pct, text) {
  const fill=$("progressFill"), pctEl=$("progressPct"), txtEl=$("progressText");
  if(fill)  fill.style.width  = `${Math.min(100,pct)}%`;
  if(pctEl) pctEl.textContent = `${Math.round(pct)}%`;
  if(txtEl) txtEl.textContent = text||"";
}
...
function showNote(type, msg) {
  const el=$("setupNote"); if(!el) return;
  el.innerHTML=msg||"";
  el.className="note "+(type==="ok"?"isOk":"isErr");
}

function showSavedFolder(dir) {
  const box=$("savedFolderBox"), lbl=$("savedFolderLabel");
  if(box) box.style.display="block";
  if(lbl) lbl.textContent=dir;
}

function showChosenFolder(dir) {
  const el=$("chosenFolderDisplay"); if(!el) return;
  el.classList.remove("isHidden");
  el.textContent=dir;
}
...
// ─── Supabase session (THE CORRECT SOURCE OF TRUTH) ──────────────────────────
async function loadSupabaseSession() {
  /**
   * Read the real Supabase session.
   * Supabase stores tokens internally — we MUST use getSession() to get them.
   * Never read from localStorage directly for auth tokens.
   */
  try {
    const { data: { session } } = await supabaseClient.auth.getSession();
...
function getSessionPayload(supaSession) {
  /** Convert Supabase session to the payload format used by Flask */
...
async function pushSession() {
  /** Send session to local bootstrap (for config.json) */
...
async function openLocalAppWithSession() {
  /**
   * Open basira_app.html with session already active.
   *
   * HOW: Navigate to http://127.0.0.1:5000/login?token=XXX&user_id=YYY
   * Flask handles /login as a same-origin GET, sets a real session cookie,
   * then redirects to / (basira_app.html).
   * basira_app.html calls /api/session/bootstrap → authenticated: true ✓
   *
   * This is the ONLY method that works correctly for cross-origin session linking.
   */
  const session = _supaSession || await loadSupabaseSession();
  const p = getSessionPayload(session);

  if (!p) {
    // No Supabase session at all — open app, it will show login overlay
    window.open(LOCAL_APP, "_blank");
    return;
  }
  ...
  // This navigates the browser to Flask /login endpoint.
  // Flask sets cookie + redirects to basira_app.html
  window.open(`${LOCAL_APP}/login?${params.toString()}`, "_blank");
}
...
async function sendHeartbeat() {
  try {
    const r=await fetch(`${LOCAL_BOOTSTRAP}/api/auth/heartbeat`,
      {method:"POST",headers:{"Content-Type":"application/json"}});
    if(r.status===401) setTimeout(()=>{ window.location.href="./login.html"; },1500);
  } catch {}
}
...
async function browseFolder(inputId) {
  try {
    const r=await apiGet("/api/system/pick-data-dir",30000);
    if(r?.status==="ok"&&r.path){ const el=$(inputId); if(el) el.value=r.path; }
  } catch { showNote("err","Could not open folder picker."); }
}
...
async function runSetup(dataDir) {
  ...
  const steps=[
    { pct:5,  status:"Initializing config...",
      fn:()=>apiPost("/api/setup/init",{},10000) },

    { pct:12, status:"Linking login session...",
      fn:pushSession },

    { pct:22, status:`Creating folders in: ${dataDir}`,
      fn:async()=>{
        const r=await apiPost("/api/setup/select-data-dir",{data_dir:dataDir},15000);
        if(!r||r.status!=="ok") throw new Error(r?.message||"Failed to create folders.");
        showChosenFolder(dataDir);
        showSavedFolder(dataDir);
        if($("savedFolderBox")) $("savedFolderBox").style.display="block";
      }
    },

    { pct:35, status:"Downloading files from GitHub...",
      fn:async()=>{
        let elapsed=0;
        ...
        try {
          const r=await apiPost("/api/setup/download-files",{},300000);
          clearInterval(si);
          if($("dlSpeed")) $("dlSpeed").textContent="";
          if(!r||r.status!=="ok")
            throw new Error(r?.message||"Download failed. Check internet connection.");
        } catch(e){ clearInterval(si); if($("dlSpeed")) $("dlSpeed").textContent=""; throw e; }
      }
    },
    ...
    { pct:93, status:"Saving configuration...",
      fn:async()=>{
        const r=await apiPost("/api/setup/finalize",{},10000);
        if(!r||r.status!=="ok") throw new Error("Failed to finalize setup.");
      }
    },
  ];
  ...
  try {
    ...
    showNote("ok","✓ Basira setup complete. Opening app...");

    // Open the local app with session embedded in URL
    setTimeout(()=>{ openLocalAppWithSession(); },1000);
...
async function initialize() {
  ...
  // Always load Supabase session first
  await readCloudUser();
  ...
  switch(state) {
    ...
    case "healthy_with_optional_update": {
      ...
      showCard("readyCard");
      if($("readySub")) $("readySub").textContent="Basira is ready. Opening app...";
      showNote("ok","Environment ready. Opening Basira...");
      // Open with session after short delay
      setTimeout(()=>{ openLocalAppWithSession(); },600);
      break;
    }
    ...
    case "recovery_required": {
      showCard("recoveryCard"); setStep(1);
      const rt=$("recoveryText"),rb=$("repairBtn"),rf=$("recoveryPathField");
      if(reason==="data_dir_missing"){
    ...
    case "update_required": {
      showCard("recoveryCard");
      const rt2=$("recoveryText"),rb2=$("repairBtn");
...
async function handleRepair() {
  const mode=$("repairBtn")?.dataset.mode||"";
  if(mode==="update"){ window.open(CLOUD_RENEW_URL,"_blank"); return; }
  if(mode==="retry"||mode==="reselect"){
    const dir=$("recoveryDir")?.value.trim()||"C:\\BasiraData";
    await runSetup(dir); return;
  }
  if(mode==="repair-files"){
    ...
    try {
      ...
      setProgress(80,"Verifying..."); setDlStatus("Verifying files...");
      const v=await apiGet("/api/setup/verify",15000);
      ...
      setTimeout(()=>{ openLocalAppWithSession(); },700);
    } catch(err){ showCard("recoveryCard"); showNote("err",err.message||"Repair failed."); }
  }
}
...
async function renewDemo() {
  try {
    ...
    setTimeout(initialize,800);
  } catch(err){ showNote("err",err.message||"Could not renew."); }
}
